...
- 個人のクレジットカードでGoogle Cloudに登録し、新規登録時に提供される **$300分・90日間の無料クレジット**（Welcome credit）を使って学習を進めます。トライアル終了時は自動課金ではなく、アカウントが自動的にクローズされる仕組みです。90日という期限は全16回（9月〜1月）の授業期間より短いため、**本科目では有料アカウントへの切り替えは行わず**、クラウドの実操作は第13回頃までのトライアル期間内で終える計画とします。トライアル終了後の回は、それまでに残したスクリーンショット・実行ログ・録画をもとに検証・発表を進めます。
...
| 2 | **Google Cloudアカウントを作ろう** | 個人のクレジットカードでGoogle Cloudに新規登録し、$300分・90日間の無料クレジット（Welcome credit）を確認する。トライアル終了時は自動課金ではなく自動的にアカウントがクローズされる仕組みで、90日という期限が全16回の授業期間より短いことも共有する。本科目では有料アカウントへの切り替えは行わず、第13回頃までのトライアル期間内でクラウドの実操作を終える計画であることを伝えておく。Gemini Enterprise Agent Platform（旧Vertex AI）の全体像を紹介する。 |
...
| 12 | **課金を可視化しよう** | Budgets & Alertsを設定し、サービス単位でのコスト状況を確認する習慣を身につける。Budgets & Alertsは通知機能であり、支出や課金を自動的に止めるものではないことを理解する。無料クレジットの残高・残り日数を確認し、トライアル終了が近いことを踏まえて、以降はスクリーンショット・実行ログ・録画で作業成果を記録に残す準備を始める。 |
| 13 | **検証と改善** | 実装した機能のテスト・動作確認。Playwright MCPをClaude Codeに追加し、自分が作ったWebアプリをAIエージェントに実際に操作・確認させる。無料トライアル終了が近い場合は、動作の様子をスクリーンショットや録画で記録しておく。 |
| 14 | **責任あるAI開発（クラウド版）** | クラウド環境でのAI活用における検証・品質管理・データガバナンスの観点から、自分の実装を見直す。 |
| 15 | **最終仕上げ** | 進級制作の最終調整、ドキュメント整備。無料トライアルが終了している場合は、記録した実行結果・スクリーンショットをもとに整理する。 |
| 16 | **🎤 最終発表会＆総括** | クラウド機能を統合した進級制作の成果発表（無料トライアル終了後はスクリーンショットや録画を用いる）。前期・後期を通じた技術の進化を振り返る。 |
...
## 14. その他（履修上の注意点）
- Google Cloudの利用には **個人のクレジットカード** が必要です（新規登録者には$300分・90日間の無料クレジットが提供され、明示的にアップグレードしない限り自動課金は発生しません）。90日間は全16回の授業期間より短いため、**本科目では有料アカウントへの切り替えは行いません**。トライアル終了後はリソースにアクセスできなくなるため、終了前に成果を記録（スクリーンショット・実行ログ・録画）しておく運用とします。
