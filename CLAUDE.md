...
program-basics-2/       # プログラム基礎Ⅱ の各回詳細ページ
  ...
  week03.html           #   第3回 データを扱う基礎②：フォーム入力を受け取る
...
| 秋の集中講座 | `autumn-intensive.html` | `page-header--slate` | 時間割枠外（全5日間） | 学科横断・既存の進級制作チーム単位（2026-09-24改訂: 新規混成チーム編成から既存進級制作チーム参加へ、カメラ×AI単一パスからトラックA「カメラ×AI」／トラックB「同時対戦ゲーム」／トラックC「自由テーマ」の3択へ。2026-09-25改訂: 他教員（複数名）案を統合、学年別の完成目標・3年次以上の下級生チームサポート・ピックアップ発表・1〜5分規模方針を追加。進級制作チームは学年混成ではないため「学科・学年をまたいだ」表現は削除済み） |
...
| ITパスポート テクノロジー系 | **グローバルIT 2年次（全員が外国人留学生）** | なし | — | 授業計画は教科書の章順ではなく**出題頻度順**に再構成済み。留学生向けの共通ルールは次節「留学生向け科目の共通ルール」を参照。2026-09-16に分野横断ストーリー型へ全面改訂（PR: it-passport-story-based-redesign）——**受験申込・予約締切の授業内管理は撤廃**し、実際の受験タイミングは各自に委ねる（試験はほぼ毎日実施、ただし12/28以降一時停止）。**全19回（9/30〜2/10）、第12回でテクノロジ系の全範囲が完走**（12/28の停止より前）。第13回は弱点補強、第14-16回はストラテジ系・マネジメント系の総復習＋総合演習、第17回期末テスト、第18-19回は日本語発表（16回前提で週ページを設計しない） |
...
### 留学生向け科目の共通ルール（ITパスポート テクノロジー系で初出）

対象学生が**全員外国人留学生**の科目（現状はITパスポート テクノロジー系のみ）に適用する共通ルール。既存サイトには翻訳配慮の実装が一切ないため（全ページ `lang="ja"` 固定、`translate="no"` 等はゼロ件）、この科目が初出の設計になる。週ページ作成時に必ず踏襲すること。

1. **用語は日本語のまま覚えさせる**: 本番のITパスポート試験は日本語で実施される。Chrome翻訳（対象9言語すべて対応済み）でページ全体を母語に翻訳させると、覚えるべき日本語の専門用語まで訳されてしまう。試験に出る用語は `<span translate="no">…</span>` で翻訳から保護し、ページ冒頭に「用語は日本語のまま覚える」案内を置く
2. **用語カード**: 各週ページに「用語（`translate="no"`）／ふりがな／やさしい日本語／English」の固定セクションを置く。English列は ITPEC 英語版過去問（`itpec.org/pastexamqa/ip.html`、教育目的なら許諾・使用料不要、出典表記必須）の表記に合わせる
3. **二層テスト**: 成績に関わる評価（親ページ評価方法30%「授業内ミニテストの到達度」等）はGoogleフォーム→スプレッドシート（i-seifu.jpドメイン限定、回答はドライブ内に閉じる）を使う。公開サイトに載せるのはフォームのURLのみで、`target="_blank" rel="noopener"` の別タブリンクで開く（iframe埋め込みは前例なし・使わない）。**サイト内JSクイズ（記録なし・個人情報なし・成績に含めない即時フィードバック用）はITパスポート テクノロジー系で採用決定済み**（2026-09-16、分野横断ストーリー型への改訂時。成績評価はGoogleフォームのみが対象で、サイト内クイズは含めない）。実装は他の週コンテンツと同じく各週ページ作成時に1回分ずつ行うため、親ページ改訂時点ではどの週ページも未作成でありサイト内クイズも存在しない。週ページ作成が進んでもなお導入しない判断に変わった場合は、この行を「検討したが不採用」に書き換える
...
- **「新規ファイルを作る」「コード例を書く」指示をする週ページでは、親ページとの整合性だけでなく、同科目の既存週ページ（特に類似の操作をしている回）との実装パターンの一貫性も執筆前に確認する**——親ページ正本原則は「親ページ vs 週ページ」の整合性をカバーするが、「兄弟週ページ同士」の実装レベルの一貫性は別軸でチェックが必要。過去事例: PR #140 で、第11回の新規ファイル `click-game.html` の最小限HTML例が `<h1>` 断片のみで、第1回で教えた `<!DOCTYPE html>`/`<html>`/`<head>`/`<body>` の骨組みと、第9回 `list.html` 作成時の完全テンプレート提示パターンに反していた（生徒が「新規ファイルは断片だけでよい」と誤解するリスクがあった）
...
| Simple Browser を `http://localhost:8000/xxx.html` 直打ち / アドレスバー全体の書き換えで案内する | Codespaces のポート転送経由 Simple Browser の URL は `https://<token>-8000.app.github.dev/` 形式で、`localhost:8000` 直打ちは成立しない。サブページに移動するときは **① `index.html` 等から `<a href="xxx.html">` リンクを追加してリンク経由で遷移、または ② Simple Browser のアドレスバーの**末尾だけ** `/xxx.html` に書き換える、のどちらか**で案内する。`http://localhost:8000/...` のような URL 全体の指示は禁止。**適用範囲はサイト内ページ間リンクだけでなく、授業本文で「学生に新規ファイル（`demo.html`等）を作らせてSimple Browserでプレビューさせる」指示を書くときも同じ**——`python3 -m http.server` はディレクトリのルート（既存の`index.html`等）を表示するため、非indexファイルへは②の手順が必須 | PR #119, PR #164 |
...
| 新規ファイル作成の指示で `<h1>` 等の断片のみをコード例として示し、`<!DOCTYPE html>`/`<html>`/`<head>`/`<body>` の骨組みを省略する（program-basics 等 HTML を書く科目） | 新規ファイル作成の指示は**必ず完全なHTML構造（`<!DOCTYPE html>`から`</html>`まで）をテンプレートとして提示する**。第1回でHTML基本構造を正式に教え、第9回の `list.html` 作成時も完全なテンプレートを使っている前例と一貫させる。省略すると「新しいファイルは断片だけでよい」という誤ったメンタルモデルを生む。`<script>` を使う回はテンプレート内に空の `<script>` 枠も用意し、本文中で「テンプレートの中身を次のコードに置き換える」と参照する | PR #140 |
| ITパスポート試験の実施時期を「随時受験できる」と無条件に書く | IPAのシステムリプレースに伴い**2026年12月28日以降、実施が一時停止**（会場によっては12/27より前に休止、再開時期は2026年秋頃に告知予定）。ITパスポート テクノロジー系は2026-09-16の分野横断ストーリー型改訂で**受験申込・予約の授業内管理を撤廃**し、実際の受験タイミングは学生の任意とする方針に転換した（試験はほぼ毎日実施され、受験する学生ごとにタイミングが異なるため）。ただし**テクノロジ系の全範囲は第12回（12/28停止より前）に完走**させ、年内受験を希望する学生が情報不足で機会を逃さないよう、参考文献に受験申込手順ページのリンクと停止時期の注意喚起は残す | 2026-09 執筆時点、2026-09-16改訂 |
| ITパスポート試験の出題分野を現行の「ストラテジ系・マネジメント系・テクノロジ系」で固定的に書く／再編後の分野名を断定する | IPA公式が**2027年度から新試験制度への移行を明言**しているが、再編後の分野名（報道されている「ビジネス・テクノロジ・セキュリティ倫理」等）は一次ソース未確認。現行制度の分野名は**シラバスVer.6.5時点の名称**と明記し、再編後の名称は公式シラバス案が確定してから裏取りする | 2026-09 執筆時点 |
...
| GitHub Codespaces上で動くサーバー（WebSocket等）の到達範囲を「学校WiFi等のローカルネットワーク限定」「NATにより外部からアクセスできない」と説明する | Codespacesのポート転送を**「パブリック」に設定すると、ローカルネットワークに限らずインターネット上の誰からでも認証なしにアクセス可能**（公式: `docs.github.com/en/codespaces/developing-in-a-codespace/forwarding-ports-in-your-codespace`「After you make a port public, anyone who knows the URL and port number can view the running application without needing to authenticate.」／`docs.github.com/en/codespaces/reference/security-in-github-codespaces`「Publicly forwarded ports: Are accessible on the internet, and anyone on the internet can access them. No authentication is needed to access public forwarded ports.」）。NATは「ふつうのルーター内側のPC」には当てはまるが、**Codespacesのポート転送はその壁の外側まで届く橋（プロキシ）を用意しているため前提が異なる**。初回アクセス時にGitHubが警告画面（「You are about to access a development port served by someone's codespace」、Continueボタン付き、**セッションごとに1回だけ表示**）を挟むが、これは注意喚起であって防御ではなく、誰でも先に進めてしまう。GitHub Pages（静的ファイル専用ホスティング）とは公開の性質が根本的に異なる点も対比すると理解しやすい（公式: `docs.github.com/en/pages/getting-started-with-github-pages/about-github-pages`「GitHub Pages is a static site hosting service that takes HTML, CSS, and JavaScript files straight from a repository」） | 自然言語開発Ⅱ week04再構成（2026-09-24、PR #228。実際の授業のゲームがCodespacesのPublicポート転送で動いており、当初のNAT中心の説明はそもそも起きていなかった誤った前提だったと判明） |
