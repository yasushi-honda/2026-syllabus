...
| Codespaces上のAntigravity CLI（agy）を複数回の授業で使い続ける週ページで、Codespace再開（resume）時にagyの認証が引き継がれる前提で書く | 公式リポジトリのIssue（google-antigravity/antigravity-cli #10、closed）で「コンテナを再起動するたびにOAuthログインをやり直す必要がある」報告あり（原因はagyが`org.freedesktop.secrets`ベースの認証情報保存を使うため、環境によっては保持されない）。断定はできないため「**再度ログインを求められる場合がある**」と条件付きで書き、第◯回と同じ再認証手順への参照を添える。Codespacesの停止→resumeを挟む週（ai-programming-2/natural-language-dev-2/ai-communication-2のagy系週）は毎回この可能性を考慮する | AI活用プログラミング基礎Ⅱ week04作成（2026-09-23） |
