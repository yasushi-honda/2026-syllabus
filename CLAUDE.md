...
| ITパスポート テクノロジー系 | **グローバルIT 2年次（全員が外国人留学生）** | なし | — | 授業計画は教科書の章順ではなく**出題頻度順**に再構成済み。留学生向けの共通ルールは次節「留学生向け科目の共通ルール」を参照。12/28以降ITパスポート試験が実施停止するため年内受験前提で書く。**全19回（9/30〜2/10）、第14〜19回は試験後の総復習・発展・日本語発表期間**（16回前提で週ページを設計しない） |
...
3. **二層テスト**: 成績に関わる評価（親ページ評価方法30%「授業内ミニテストの到達度」等）はGoogleフォーム→スプレッドシート（i-seifu.jpドメイン限定、回答はドライブ内に閉じる）で実装済み。公開サイトに載せるのはフォームのURLのみで、`target="_blank" rel="noopener"` の別タブリンクで開く（iframe埋め込みは前例なし・使わない）。**サイト内JSクイズ（記録なし・個人情報なし・成績に含めない即時フィードバック用）は週ページ作成時に追加検討する未実装の拡張案**であり、親ページ段階では存在しない。週ページでこれを導入しない判断をした場合は、この行を「検討したが不採用」に書き換える
...
| ITパスポート試験の実施時期を「随時受験できる」と無条件に書く | IPAのシステムリプレースに伴い**2026年12月28日以降、実施が一時停止**（会場によっては12/27より前に休止、再開時期は2026年秋頃に告知予定）。ITパスポート テクノロジー系のように受験を前提とする科目は、**年内受験を前提にカリキュラムを設計**し、直前対策回を12月中に置く | 2026-09 執筆時点 |
